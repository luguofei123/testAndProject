window.onload = function(){
	...
	window.EmployeeView = Backbone.View.extend({
		...
		events : {
			"dblclick td" : "edit",  //双击事件
			"blur input,select" : "close",//失去焦点事件
			"click .del" : "clear",   //删除事件
		},
		initialize : function(){
			// 每次更新模型后重新渲染
		...
		setText : function(){
			...
			this.input = $(this.el).find('input,select'); //alert(this.input.length)
			this.input.each(function(){
				var input = $(this);
				input.val(model.get(input.attr("name")));//通过获取input输入的值
			});
		},
		close: function(e) {
